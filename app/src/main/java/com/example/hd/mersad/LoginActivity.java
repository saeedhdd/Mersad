 ...
 */
public class LoginActivity extends FatherActivity implements LoaderCallbacks<Cursor> {
...
//Remove notification bar
        ...
        mEmailView = (AutoCompleteTextView) findViewById(R.id.signup_username);
...
//        mPasswordView.setTypeface(typeface);
        ...
        imageView =
                (ImageView) findViewById(R.id.mersadLogo);
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            String json = "  {\n" +
                    "    \"username\": \""+mEmailView.getText()+"\",\n" +
                    "    \"password\": \""+mPasswordView.getText()+"\"\n" +
                    "      }";
            final RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"),json);
            MyApp.myApi.postLoginCredentials(requestBody).enqueue(new Callback<LoginPost[]>() {
                @Override
                public void onResponse(Call<LoginPost[]> call, Response<LoginPost[]> response) {
                    if (response.body().length !=0) {
                        LoginPost loginPost = response.body()[0];
                        User.user.username = mEmailView.getText().toString();
